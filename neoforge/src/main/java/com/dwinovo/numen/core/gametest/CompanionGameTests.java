 ...
 * 会被 {@code NbtUtils.unpackStructureTemplate} 静默丢弃,一块不放)。
 *
 * <p>坐标口径:{@link GameTestHelper#absolutePos} 本代以<b>测试原点</b>(结构方块 + 结构
 * 偏移,比结构方块高一格)为基准,而模板方块也落在 {@code 测试原点+rel}——两者对齐,
 * {@code absolutePos(rel)} 就是模板里 rel 那一格。上一代 absolutePos 以结构方块为基准,
 * 比模板低一格,用例坐标因此整体差一格。为让用例坐标在两代之间一字不改,本代的模板在
 * 最底下垫了一层空气(地面从模板 rel y=0 挪到 rel y=1,size.y 相应 +1)——于是两代都是
 * {@code absolutePos(y=1)} 是地面、{@code absolutePos(y=2)} 是站立面。
 *
 * <p>这一格错位不会报错也不会少断言:场地夹具与断言全走 absolutePos,自洽;只有像
 * "手摆的墙必须坐在模板地面上"这类跨来源对齐的用例会现形(门那两条就是)。
 */
public class CompanionGameTests {
    ...
    // 旧的 String 口径,158 处断言的写法与其余版本分支逐行可比,只是接收者从
    // helper 挪到了首参。
    ...
    // 结构模板目录的设置已移到 NumenGameTests.register():本类的方法表是被反射读的,
    // 而反射读方法不触发静态初始化,静态块在本代一次都不会跑。
    ...
    public static void build_block_ids_and_material_counts(GameTestHelper helper) {
        ...
        assertTrue(helper, com.dwinovo.numen.core.tools.BuildPalette.parse("dirt_path")
    ...
    public static void blueprint_block_entity_contents_survive(GameTestHelper helper) {
        ...
        assertTrue(helper, com.dwinovo.numen.core.task.BuildStates.safeBlockEntityData(
        ...
        assertTrue(helper, com.dwinovo.numen.core.task.BuildStates
        ...
        assertTrue(helper, com.dwinovo.numen.core.task.BuildStates.safeEntityData(
    ...
    public static void quote_and_gate_count_the_same_slots(GameTestHelper helper) {
        ...
        // 1.21.5:副手迁入 EntityEquipment,经槽位映射写入
    ...
    public static void blueprint_read_matches_what_gets_spent(GameTestHelper helper) throws Exception {
        ...
        assertTrue(helper, loaded.targets().stream().noneMatch(t -> com.dwinovo.numen.core.task
    ...
    private static void copyCottageFixture(ServerLevel level) throws Exception {
        // 1.21.5:testStructuresDir 已经是 Path
        java.nio.file.Path src =
                StructureUtils.testStructuresDir.resolve("japanese_cottage.litematic");
