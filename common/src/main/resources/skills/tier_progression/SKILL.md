...
7. **Bow + arrows**: bow = 3 sticks + 3 string (`hunt` spiders at night for string); arrows = 1 flint + 1 stick + 1 feather → 4 (flint drops from `mine(gravel)` at ~10%, feathers from chickens). Target 32 arrows — more is comfort, not requirement; melee + food covers what arrows don't.
