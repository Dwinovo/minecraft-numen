...
public final class MoveToTool implements NumenTool {
    ...
    public String description() {
        return """
                ...
                • Go to an EXACT cell: give x, y and z. It occupies exactly that cell — if something solid is there, it digs it out and stands in its place. To go USE a block (chest, crafting table, furnace, ore…) do NOT send bare coordinates (that would dig it): send its coordinates WITH arrival:'interact' (stops right beside it, never touches it), or just send block:'<its name>' for the nearest one.
                • Change ELEVATION: give y only (x and z null) to climb to the surface or descend to a mining depth at your current column.
                • Go to the NEAREST block of a kind: give block only (e.g. 'crafting_table'), no coordinates. It scans nearby, walks up beside the closest one (never touching it) and reports where it is — no locate/scan call needed first.
    ...
    public Map<String, Object> parameterSchema() {
        return Schema.object()
                ...
                .optionalString("block", "Namespaced block id to walk to the nearest of (e.g. "
                        + "'crafting_table' or 'minecraft:chest'). Give it ALONE — no coordinates. "
                        + "It stops right beside the closest match and names its position.")
                .optionalNullableEnum("arrival", "How to finish at an x+y+z target; only valid with "
                        + "all three coordinates. Your choice IS the intent, like picking a command: "
                        + "omitted/'stand_on' = occupy exactly that cell, digging out whatever is "
                        + "there. 'interact' = the cell is a block you want to USE: stop right beside "
                        + "it and never touch it — ALWAYS pick this for chests/stations/ores you'll "
                        + "work on. 'near' = anywhere within 3 blocks is fine.",
    ...
    public void onServerCall(String toolCallId, JsonObject args, NumenPlayer companion, Consumer<String> reply) {
        ...
        dispatchAsync(companion, impl.moveTo(a.x(), a.y(), a.z(), a.speed(),
                a.block(), a.arrival(), ctx(toolCallId, companion)), reply);
    }
}
