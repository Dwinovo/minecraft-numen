...
/**
 * Resolves a line-of-sight-verified hit for placing a block at {@code placeAt}. Two stages,
 * in precedence order:
 *
 * <ol>
 ...
 *   <li><b>Direct on the target</b> (fallback) — a block sitting in the target cell is assumed
 *       replaceable, so just aim AT it and place (vanilla replaces it). It samples the target's
 ...
 * the real eye) — the "lean over a ledge" reach. Clip is
 * {@code OUTLINE}.
 *
...
public final class Placement {
    ...
    /** Every neighbour we may place against — every direction except UP. */
    private static final Direction[] FACES = {
            Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST, Direction.DOWN};

    /** Down/up/N/S/W/E face-centre offsets,
    ...
    public static BlockHitResult resolve(NumenPlayer player, BlockPos placeAt, boolean wouldSneak, Double aimY) {
        double reach = 4.5D;   // 1.20.1: no Player#blockInteractionRange()
     ...
     *  shape is still hit from some angle. */
    ...
    /** A normal cube / glass presents a face (NOT every full-collision
