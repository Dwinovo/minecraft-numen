...
public final class RecoveryPolicy {
    ...
    public record Candidate(
        ...
        public static Candidate cleansing(int slot) {
            return new Candidate(slot, Kind.MILK, Value.ORDINARY, 0, 0.0f, 0.0f);
        }
    }
    ...
    public static Decision decide(State state, List<Candidate> candidates) {
        ...
        if (needsActiveHealing) {
            if (state.cleanseRecommended()) {
                int instantSlot = state.health() <= 4.0f
                    ? bestHealingSlot(
                        candidates,
                        Kind.INSTANT_HEALTH,
                        Math.max(0.0f, state.maxHealth() - state.health())
                    )
                    : -1;
                if (instantSlot >= 0) {
                    return new Decision(Mode.ACTIVE_HEALING, instantSlot, 4.5f);
                }
                int milkSlot = milkSlot(candidates);
                if (milkSlot >= 0) {
                    return new Decision(Mode.CLEANSE, milkSlot, 4.2f);
                }
            }
        ...
        boolean cannotNaturallyHeal = state.health() < state.maxHealth()
            && state.foodLevel() < 18;
        if (state.naturalRegeneration()
            && state.foodLevel() < 20
            && (state.health() <= 12.0f || cannotNaturallyHeal)) {
    ...
    public static boolean allowsRequestedUse(
        ...
        return (decision.mode() == Mode.ACTIVE_HEALING || decision.mode() == Mode.CLEANSE)
    ...
    private static int milkSlot(List<Candidate> candidates) {
        return candidates.stream()
            .filter(candidate -> candidate.kind() == Kind.MILK)
            .mapToInt(Candidate::slot)
            .min()
            .orElse(-1);
    }

    public static int selectFoodSlot(Mode mode, State state, List<Candidate> candidates) {
        if (mode == Mode.NONE || mode == Mode.ACTIVE_HEALING || mode == Mode.CLEANSE) {
            return -1;
        }
