...
public final class TakeItemsTool implements NumenTool {
    ...
    public void onServerCall(String toolCallId, JsonObject args, NumenPlayer companion, Consumer<String> reply) {
        ...
        // Math.clamp 是 Java 21 API,本分支编译级别 Java 17,用原版的 Mth.clamp。
        int want = net.minecraft.util.Mth.clamp(a.count() == null ? 1 : a.count(), 1, MAX_COUNT);
        // 按满栈分批塞;背包塞不下的原版 add 会留在栈里,掉在脚下(与描述一致)
        int remaining = want;
        while (remaining > 0) {
            int n = Math.min(remaining, item.getMaxStackSize());
