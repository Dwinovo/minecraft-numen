...
public final class IntentPinsData extends SavedData {
    ...
    // Same era-appropriate registration as CompanionRegistry: the old
    // SavedData.Factory (supplier + deserializer + fix type), codec-driven by hand.
    private static final SavedData.Factory<IntentPinsData> FACTORY = new SavedData.Factory<>(
            IntentPinsData::new, IntentPinsData::load,
    ...
    private static IntentPinsData load(CompoundTag tag, HolderLookup.Provider registries) {
    ...
    public static IntentPinsData get(MinecraftServer server) {
        return server.overworld().getDataStorage().computeIfAbsent(FACTORY, "numen_intent_pins");
    }
