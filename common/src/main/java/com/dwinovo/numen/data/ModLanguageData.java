...
public final class ModLanguageData {
    ...
    private static void addEn(Adder adder) {
        ...
        adder.add(Keys.VOICE_FORM_URL,       "Service URL (prefilled; only change for a custom endpoint)");
        adder.add(Keys.VOICE_FORM_KEY_OPENAI,  "API Key — provider console (sk-…)");
        ...
        adder.add(Keys.VOICE_FORM_MINIMAX_MODEL, "Model (blank = speech-02-turbo; newer: speech-2.6-turbo/hd)");
        adder.add(Keys.VOICE_FORM_MINIMAX_VOICE, "voice_id, e.g. male-qn-qingse (300+ system voices)");
        adder.add(Keys.VOICE_FORM_GROUP,     "GroupId — console account page (fill it if the API errors ask for it)");
        adder.add(Keys.VOICE_FORM_REFERENCE, "reference_id — paste the voice page URL or the ID after /m/ (blank = account default)");
        adder.add(Keys.VOICE_FORM_FISH_MODEL, "Model: s1 / s2-pro / s2.1-pro / s2.1-pro-free (blank = default)");
        adder.add(Keys.VOICE_FORM_MODEL,     "Model id, e.g. FunAudioLLM/CosyVoice2-0.5B");
        adder.add(Keys.VOICE_FORM_VOICE,     "Voice = model:name, e.g. FunAudioLLM/CosyVoice2-0.5B:alex");
        adder.add(Keys.VOICE_FORM_REF,       "Reference audio path (on the machine running GPT-SoVITS)");
        adder.add(Keys.VOICE_FORM_PROMPT,    "Transcript of that reference audio");
        adder.add(Keys.VOICE_FORM_LANG,      "Language zh/en/ja (blank = zh)");
    ...
    private static void addZh(Adder adder) {
        ...
        adder.add(Keys.VOICE_BACKEND_MINIMAX, "MiniMax");
        adder.add(Keys.VOICE_BACKEND_FISH,    "Fish Audio");
        adder.add(Keys.VOICE_FORM_URL,       "服务地址(已预填官方端点,自建/代理才需要改)");
        adder.add(Keys.VOICE_FORM_KEY_OPENAI,  "API Key——服务商控制台创建(sk-…)");
        ...
        adder.add(Keys.VOICE_FORM_MINIMAX_MODEL, "模型(留空即 speech-02-turbo,新版有 speech-2.6-turbo)");
        adder.add(Keys.VOICE_FORM_MINIMAX_VOICE, "voice_id,如 male-qn-qingse(平台音色库 300+ 可选)");
        adder.add(Keys.VOICE_FORM_GROUP,     "GroupId——账户管理页复制(接口报错要求时必填)");
        adder.add(Keys.VOICE_FORM_REFERENCE, "reference_id——直接粘贴声线页网址或填 /m/ 后那串 ID 都行(留空用账号默认)");
        adder.add(Keys.VOICE_FORM_FISH_MODEL, "合成模型:s1/s2-pro/s2.1-pro/s2.1-pro-free(留空用默认)");
        adder.add(Keys.VOICE_FORM_MODEL,     "模型 id,如 FunAudioLLM/CosyVoice2-0.5B");
        adder.add(Keys.VOICE_FORM_VOICE,     "音色=模型:音色名,如 FunAudioLLM/CosyVoice2-0.5B:alex");
        adder.add(Keys.VOICE_FORM_REF,       "参考音频路径(GPT-SoVITS 所在机器上的路径)");
        adder.add(Keys.VOICE_FORM_PROMPT,    "参考音频说的那句话(文本)");
        adder.add(Keys.VOICE_FORM_LANG,      "语言 zh/en/ja(留空默认 zh)");
