...
public final class ModLanguageData {
    ...
    private static void addEn(Adder adder) {
        ...
        adder.add(Keys.VOICE_FORM_GROUP,     "GroupId (optional, legacy endpoints)");
        adder.add(Keys.VOICE_FORM_REFERENCE, "Voice model reference_id");
        adder.add(Keys.VOICE_FORM_FISH_MODEL, "Model header, e.g. s1 (optional)");
    ...
    private static void addZh(Adder adder) {
        ...
        adder.add(Keys.VOICE_FORM_GROUP,     "GroupId(可选,旧版接入需要)");
        adder.add(Keys.VOICE_FORM_REFERENCE, "声线模型 reference_id");
        adder.add(Keys.VOICE_FORM_FISH_MODEL, "模型 header,如 s1(可留空)");
