...
public final class ScaffoldCleanupGateTest {
    ...
    void verifiedRuntimeBehavior() {
        ...
        require(
            ScaffoldCleanupGate.canRun(false, false, false, false, false, false, false),
            "cleanup should run only when every task and path lane is idle"
        );
        require(
            !ScaffoldCleanupGate.canRun(false, false, true, false, false, false, false),
            "an active asynchronous task must block cleanup even if the brain running field is empty"
        );
        require(
            !ScaffoldCleanupGate.canRun(false, false, false, true, false, false, false),
            "an active agent turn must block cleanup between consecutive tools"
        );
        require(
            ScaffoldCleanupGate.canContinueRetreat(false, false, false, false),
            "cleanup-owned retreat navigation should continue while task lanes stay idle"
        );
        require(
            !ScaffoldCleanupGate.canContinueRetreat(true, false, false, false),
            "a new task chain must preempt cleanup-owned retreat navigation"
        );
        require(
            !ScaffoldCleanupGate.canContinueRetreat(false, true, false, false),
            "a newly queued task must preempt cleanup-owned retreat navigation"
        );
        require(
            !ScaffoldCleanupGate.canContinueRetreat(false, false, false, true),
            "a resumed agent turn must preempt cleanup-owned retreat navigation"
        );
    }
