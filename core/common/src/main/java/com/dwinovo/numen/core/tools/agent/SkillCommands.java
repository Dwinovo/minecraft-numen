 ...
 * 提升为快捷工具 {@code load_skill}:名字、参数({@code name}、{@code file})与描述照旧,模型看到的工具一字不变;
 * 快捷工具与 {@code skill load} 是同一个处理函数、同一份回执。
 */
public final class SkillCommands {
    ...
    private static void load(ClientSource src, CommandArgs args) {
        src.reply(SKILLS.loadSkill(args.get(NAME), args.get(FILE)));
    }
}
