...
public final class MoveToTool implements NumenTool {
    ...
    public String description() {
        return """
                ...
                TERRAIN: the walk never changes the world unless you say so — walls, floors, other people's builds and the landscape stay exactly as they were. When there is no clean route, the call FAILS and lists candidate routes, each with an id, its length and exactly which blocks it would break or place; read them (planks/glass/bricks near the surface are usually someone's build; stone/dirt underground usually are not), then either goto route:<id> or pick another destination. Underground travel and climbing out of pits usually need alter:'natural'. Every call reports what it actually broke or placed.
                SPEC (optional object, every field optional): alter 'none'|'natural' — may the walk dig, bridge, pillar; avoid — cell types to keep out of (water, door, ...); penalties {place, break, jump, wade} — make an action pricier so she detours instead; avoid_break / avoid_place / avoid_step — blocks (ids or #tags) or cells/boxes she must not break, place into, or stand on; parkour — running jumps over gaps; climb_vines; max_fall — highest drop without water; max_alterations — cap on blocks changed.
