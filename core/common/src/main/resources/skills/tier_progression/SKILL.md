...
3. **Food**: scan cows/pigs/chickens and pass their runtime IDs to `attack` (6+ total) → cook the raw meat: `use block` a furnace, `transfer` the raw food into the top slot + fuel (coal or planks) below, then `wait` and `transfer` the cooked food out (see the `containers` skill). Always cook; raw meat barely heals.
