---
name: containers
description: How to move items in/out of any container or machine GUI — chest, barrel, shulker, furnace, modded machine. The open (use block) → use gui → transfer → use close loop, depositing/taking/swapping with the transfer tool, crafting by laying a recipe into the grid with transfer, smelting by loading a furnace, and error recovery.
---
...
1. **Open** — `use block right <x> <y> <z>` on the container block (goto it first: it does not travel). This opens its GUI and leaves it open.
...
4. **Verify** — the transfer result already tells you what happened; `use gui` again only if you need to re-check.
...
- **`from`** — the source slot (from `use gui`).
...
1. **`inv recipe <item>`** — get the ingredients and, for shaped recipes, the grid layout.
2. **Open the grid:**
   - **≤2×2 recipe** (planks, sticks, torches, a crafting table): NO table needed — `use gui` with nothing open shows your own 2×2 grid.
   - **3×3 recipe** (most tools, etc.): `use block right <x> <y> <z>` a crafting table, then `use gui` — it draws the grid as a 2D map of slot numbers.
...
*Example — sticks (2 oak_planks stacked vertically):* `use gui` (own grid, say cells are slots 1–4 in a 2×2) → `transfer moves=[{from:<planks>, to:1, count:1}, {from:<planks>, to:3, count:1}]` → `transfer moves=[{from:<result>}]`.

## Smelting

Smelting is NOT crafting — there's no auto-tool, you load the furnace yourself (it's just two slots):
  1. `use block right <x> <y> <z>` the furnace / blast furnace / smoker.
  ...
  4. `use close`, then the command `task timer <seconds> <reason>` for roughly when the batch should be done — a vanilla furnace takes ~10s per item, a blast furnace / smoker ~5s. The timer doesn't occupy your body, so walk away and do something else; don't stand there polling.
...
```
use block right x y z
use gui                                 (find the iron stack + a free inventory slot)
transfer moves=[{from:<iron>, to:<free>, count:10}]
use close
```

**Empty a furnace's output:** `use gui` → the `[output]` slot → `transfer moves=[{from:<output>}]`.

## Error recovery (your advantage)

The transfer result tells you the outcome of every move, and you can always `use gui` — you are never blind:
