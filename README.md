...
2. **填入 API key。** 按 **`N`** → **设置** → **模型**，选一家，粘贴你自己的 key。
...
> **面板**：按 `N` 有三页——**Chat**（聊天 + 实时计划面板）、**Items**（一张仿原版背包的只读角色卡）、**Settings**。左栏是同伴名册，点头像切换、点 **`+`** 召唤、点 **`✕`** 注销，基本不用敲指令。左边缘还有个小头像 HUD，它说话时会滑出来。设置页分十项：模型、语音输入、语音输出、人格、档案、皮肤、主题、技能库、外接大脑、MCP。
