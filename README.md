...
而且它会长大：一篇 Markdown 就能教会它一套新玩法，一个插件就能把机械动力、AE2 这样的模组接到它手上——[这两样社区都能写](#扩展它)。
...
同伴是这个世界里的一具**真身体**——模组的方块它能挖能放，模组的容器它能开能拿，暴露了标准 capability 的机器它能隔着外壳读出里面装着什么。这一层装上就有，不必为谁单独适配。

它缺的是**怎么玩**。AE2 的通道要算、机械动力的应力超了整条线会停、有些东西得先升级到某一阶才有意义——这类知识不在任何数据结构里，读不出来，只能教。

教它的两样东西，**社区都能写**：

- **技能（Skill）** —— 一篇 Markdown，零代码。把你基地的规矩、某个模组的玩法写下来，丢进 `config/numen/skills/` 就生效。出厂带了一套示例（下界、烈焰棒、末影珍珠、要塞、龙战），照着改就行。
- **插件（Plugin）** —— 一个 mod，把**别的模组的能力接进来**。装上机械动力的插件，同伴就会玩机械动力；装上 AE2 的，它就懂 AE2。工具和技能可以一起随 jar 带来，玩家装上就自动学会。

技能教它怎么想，插件给它新的手。两样叠起来，它既更像个能一起玩的人，也真能干活——往远了说，总该轮到它自己造一架飞机。

> 设置的 **MCP** 一页可以临时挂一个 Model Context Protocol 服务器，那里面的工具同样一视同仁。但那是给你自己接现成服务用的：适配一个模组不能指望它自己提供 MCP 服务器，那得由插件来做。
