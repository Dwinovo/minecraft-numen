...
- 🔭 **真感知**——默认沿用扫方块、扫实体、查状态、查配方等结构化感知；玩家也可明确开启“混合”或“纯视觉”，让当前模型观察同伴自身的第一人称画面。Numen 不根据模型名称猜测它是否支持视觉。
- 🧠 **会长本事**——成功做完尚无现成 Skill 的新颖多步骤操作后，会在后台复盘真实工具反馈并自动总结成新的本地 Skill；只学习可复用流程，绝不覆盖已有技能。
...
**用哪个模型好？** 任意 OpenAI 兼容后端都行。想省钱：DeepSeek、Qwen、Kimi；想要最聪明：Claude、GPT。如果你确认所用端点支持图片，可在“设置 → 视觉”手动开启混合或纯视觉观察；Numen 不维护视觉模型名单。混合模式会用画面泛化、再用结构化工具核实数量与坐标。
