...
2. `ConsentRequestPayload` 发主人客户端:清单在服务端按"动作 + 对象 + 理由"归堆,每堆带给主人看的可翻译说法
   ("挖掉 橡木原木 ×6 · 玩家放的",方块与实体用游戏里的名字,信号有各自的译文;格子不写,世界里描轮廓)——给模型的
   回执是同一堆的英文说法(坐标最多点名 6 个,与路线账单同一种说法)。另带"允许并记住"会写进主人层的规则行、
   要描轮廓的格子与实体。
...
3. **问答挡住对话框**(与 Claude Code 的权限提示、pi 的 select 对话框同形):这只同伴的输入行被答复框取代,
   ...
   倒计时、别的同伴还有几条;四项:**允许**(这次活里同样的都行,只在发起它的任务里有效)、**以后都允许**
   (本任务内同样放行,并把选项上写着的 allow 行写进主人层,见 §七)、**拒绝**、**不行,告诉她该怎么做**
   (选中即借输入框直接打字,回车按拒绝连同这句送出,↑ 回到选项)。↑↓ 选、回车确定或直接按 1-4。
   清单里有撤不回的事时不给默认选中。Esc 只关界面,
