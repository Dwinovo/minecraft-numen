...
3. **问答挡住对话框**(与 pi 把编辑器整个换成选择框、Claude Code 的权限提示同形):这只同伴的整条输入行
   ...
   (都由输入行开)。框上是谁在问、清单(撤不回的标出来)、倒计时、别的同伴还有几条;四项:**允许**
   (这次活里同样的都行,只在发起它的任务里有效)、**以后都允许**(本任务内同样放行,并把选项上写着的 allow 行
   写进主人层,见 §七)、**拒绝**,第四项就是一个输入框(占位写着"不行,告诉她该怎么做"):选中直接打字,
   回车按拒绝连同这句送出,↑ 回到选项。这个输入框就是输入行自己那一个,屏幕上始终只有一个真输入框。
   ↑↓ 选、回车确定或直接按 1-4。清单里有撤不回的事时不给默认选中。Esc 只关界面,
   请求留着。界面外的提醒全用原版 toast,在右上角:有请求挂着是教程 toast"小蓝 想征得你的同意 / 按 Y 答复"
   (键名取自按键绑定,请求没了自己收起);有征询挂着时对话键先打开最早在等的那位的答复框,答完还有别的同伴
   在等就换到她。面板侧栏给在等的同伴头像标"!"。答完、主人没答就撤了,各一条原版系统 toast 说清答了什么
   (记下了哪几行规则)、为什么撤。
...
4. 征询协议:登记处、网络载荷、答复框、原版 toast 提醒、轮廓、超时;派发器豁免。(已落地)
