...
2. `ConsentRequestPayload` 发主人客户端:清单正文在服务端按"动作 + 对象 + 理由"归堆组好
   (坐标最多点名 6 个,与路线账单同一种说法),另带"允许并记住"会写进主人层的规则行、要描轮廓的格子与实体。
   世界里给涉及的方块和实体描轮廓,请求结束就撤。撤回带着原因(超时、任务结束、发起者撤回),主人没答就撤掉的
   用一条 toast 说清。
3. **问答挡住对话框**(与 Claude Code 的权限提示、pi 的 select 对话框同形):这只同伴的输入行被答复框取代,
   答完才回来——G 面板的聊天页与 Y 快捷对话是同一张框(都由输入行开)。框上是谁在问、清单(撤不回的标出来)、
   倒计时、别的同伴还有几条;三项:**允许**(这次活里同样的都行,只在发起它的任务里有效)、**以后都允许**
   (本任务内同样放行,并把选项上写着的 allow 行写进主人层,见 §七)、**拒绝**。↑↓ 选、回车确定或直接按 1-3;
   在选中的一项上按 Tab 借输入框写附言,随这一项一起送出。清单里有撤不回的事时不给默认选中。Esc 只关界面,
   请求留着。没开界面时 HUD 左上角一行提示条:谁在问、问的第一件事、按对话键答复、倒计时;有征询挂着时
   对话键先打开最早在等的那位的答复框,答完还有别的同伴在等就换到她。面板侧栏给在等的同伴头像标"!"。
   答完一条 toast 说清答了什么、记下了哪几行规则。
4. `ConsentReplyPayload` 带同 id 回服务端,只认主人;`/numen consent <allow|remember|deny> <id> [附言]` 与它落到
   同一个入口 `ConsentDesk.reply`。允许:清单记成任务期授权,动作继续;附言等不到任务收场,当场作为
   `consent_note` 事件(恒为急件)转给她。拒绝:动作失败,`REFUSED` 的理由是主人那句原话或"主人拒绝",随工具
   结果与 task_finished 送达,不另转一份。每句附言只走一条路。
...
模型看到的是普通的工具结果和回执,外加允许时的附言事件。系统提示词加一段:有些动作要主人点头,身体自己会问,
被拒就别绕路,允许时附的话照着做。
...
4. 征询协议:登记处、网络载荷、答复框、提示条、轮廓、超时;派发器豁免。(已落地)
