...
public class CompanionGameTests {
    ...
    public static void blueprint_handiwork_is_not_free(GameTestHelper helper) {
        ...
        var carried = com.dwinovo.numen.core.task.BuildStates.payloadStacks(frame);
    ...
    public static void only_four_item_components_ride_along(GameTestHelper helper) {
        // 一个装了东西的潜影盒挂在展示框里。本版本的物品数据是一段 NBT 而不是一组
        // 具名组件:装的东西在 tag.BlockEntityTag.Items,自定义名在 tag.display.Name。
        var box = new net.minecraft.nbt.CompoundTag();
        box.putString("id", "minecraft:shulker_box");
        box.putByte("Count", (byte) 1);
        var boxTag = new net.minecraft.nbt.CompoundTag();
        ...
        var safe = com.dwinovo.numen.core.task.BuildStates.safeEntityData(frame);
        ...
        helper.assertTrue(!carriedNbt.contains("BlockEntityTag"),
                "a container's contents must be stripped from the DATA, not just from the price"
        ...
        // 计价那一边读的是同一份
        var priced = com.dwinovo.numen.core.task.BuildStates.payloadStacks(safe);
