...
public final class NumenToasts {
    ...
    private static final int ROW_H = Math.max(AVATAR, MAX_LINES * LINE_H + PADV * 2);
    ...
    private static net.minecraft.resources.ResourceLocation spr(String n) {
        return new net.minecraft.resources.ResourceLocation(com.dwinovo.numen.Constants.MOD_ID, "textures/" + n + ".png");
    }
    ...
    public static void render(GuiGraphics g) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            renderOne(g, font, th, entries.get(i).uuid(), TOP_MARGIN + i * (ROW_H + STACK_GAP), now);
        }
    }
    ...
    private static void drawAvatar(GuiGraphics g, UUID uuid, int x, int y, UiTheme th) {
        ...
        g.blit(AVATAR_FRAME, x - 2, y - 2, 0, 0, AVATAR + 4, AVATAR + 4, AVATAR + 4, AVATAR + 4);
    ...
    private static void drawBubble(GuiGraphics g, Font font, int ax, int ay, Status s, long now) {
        ...
        int by = ay;
        g.blit(TIP_SPRITE, bx - TIP_W + 1, ay + AVATAR / 2 - TIP_H / 2, 0, 0, TIP_W, TIP_H, TIP_W, TIP_H);
        g.blit(BUBBLE_SPRITE, bx, by, 0, 0, W, h, W, h);
        int ly = by + PADV;
    ...
    private static ResourceLocation skinFor(UUID uuid) {
