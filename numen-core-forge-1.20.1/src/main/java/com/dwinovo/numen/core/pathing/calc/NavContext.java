...
public final class NavContext {
    ...
    public String diagnoseObstruction(BlockPos from, BlockPos to) {
        ...
        for (int i = 1; i <= steps; i++) {
            ...
            double fx = from.getX() + 0.5 + (to.getX() - from.getX()) * t;
            double fy = from.getY() + (to.getY() - from.getY()) * t;
            double fz = from.getZ() + 0.5 + (to.getZ() - from.getZ()) * t;
            BlockPos feet = new BlockPos((int)Math.floor(fx), (int)Math.floor(fy), (int)Math.floor(fz));
