...
public final class PlayerPathExecutor {
    ...
    /** Net-forward-progress watchdog; survives move-local reset and detects index thrashing. */
    ...
    private boolean diagonalSafeToCancel(Movement mv) {
        ...
        if (feet.equals(cornerA) || feet.equals(cornerB)) {
            ...
            return BlockHelper.canWalkOn(level, new BlockPos((int)Math.floor(x + off), (int)Math.floor(y), (int)Math.floor(z + off)))
                    || BlockHelper.canWalkOn(level, new BlockPos((int)Math.floor(x + off), (int)Math.floor(y), (int)Math.floor(z - off)))
                    || BlockHelper.canWalkOn(level, new BlockPos((int)Math.floor(x - off), (int)Math.floor(y), (int)Math.floor(z + off)))
                    || BlockHelper.canWalkOn(level, new BlockPos((int)Math.floor(x - off), (int)Math.floor(y), (int)Math.floor(z - off)));
        }
        return true;
    ...
    private Status trackProgress(Movement mv) {
        PathProgressWatchdog.Signal signal = progressWatchdog.tick(index, ticksOnCurrent);
        if (signal == PathProgressWatchdog.Signal.WARN) {
            Constants.LOG.warn("[numen-path] STUCK no forward progress {} ticks | {}",
                    progressWatchdog.ticksSinceProgress(), desc(mv));
        }
        if (signal == PathProgressWatchdog.Signal.REPLAN) {
            return replan("no net progress " + progressWatchdog.ticksSinceProgress()
                    + " ticks (index thrash)");
        }
        return null;
    }
