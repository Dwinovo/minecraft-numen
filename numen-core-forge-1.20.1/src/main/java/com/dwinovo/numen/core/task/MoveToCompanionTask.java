...
public final class MoveToCompanionTask implements CompanionTask {
    ...
    public void start() {
        ...
        // A restored record already carries its remaining cross-restart budget.
        // Fresh tasks still receive the original distance-scaled extension.
        if (!r.wasActiveBeforeRestart()) r.extendDeadlineTo(player.level.getGameTime() + extra);
    ...
    @Override
    public boolean monitorsMovementProgress() {
        return true;
    }

    @Override
    public boolean recoverFromStuck() {
        if (nav != null) nav.stop();
        settleTicks = 0;
        bestDist = repDistance();
        nav = PlayerNav.toGoal(player, this::goal, r.speed, this::reached);
        if (r.kind == MoveToTaskRecord.Kind.BLOCK) {
            nav.setHighlights(() -> java.util.List.of(blockTarget));
        }
    ...
    public TaskResult buildResult(TaskState finalState) {
        ...
        if (r.getRecoveryAttempts() > 0) data.put("recovery_attempts", r.getRecoveryAttempts());
        if (r.getFailureCode() != null) data.put("failure_code", r.getFailureCode().code());
        if (r.getFailureDetail() != null) data.put("failure_detail", r.getFailureDetail());
    ...
    private TaskResult blockedResult(int gy, String failReason, Map<String, Object> data) {
        ...
        if (!data.containsKey("failure_code")) data.put("failure_code", TaskFailureCode.UNREACHABLE.code());
