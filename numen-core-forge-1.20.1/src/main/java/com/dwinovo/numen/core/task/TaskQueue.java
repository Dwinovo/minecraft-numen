...
public final class TaskQueue {
    ...
    public void enqueueFirst(TaskRecord record) { pending.addFirst(record); }
    ...
    public boolean isEmpty() {
        return pending.isEmpty() && completed.isEmpty();
    }

    /** Shift every queued task's absolute deadline after a scheduler pause. */
    public void shiftDeadlinesBy(long ticks) {
        if (ticks <= 0L) return;
        for (TaskRecord record : pending) record.shiftDeadlineBy(ticks);
    }
    ...
    public List<TaskRecord> pendingSnapshot() {
        return List.copyOf(pending);
    }

    public TaskRecord find(String toolCallId) {
        if (toolCallId == null) return null;
        for (TaskRecord record : pending) {
            if (toolCallId.equals(record.getToolCallId())) return record;
        }
        return null;
    }

    public boolean cancel(String toolCallId, String reason) {
        var it = pending.iterator();
        while (it.hasNext()) {
            TaskRecord r = it.next();
            if (!r.getToolCallId().equals(toolCallId)) continue;
            it.remove(); r.setState(TaskState.CANCELLED); r.markFailure(TaskFailureCode.CANCELLED, reason);
            r.setResult(TaskResult.cancelled(reason, TaskFailureCode.CANCELLED.code(), Map.of("reason", reason)));
            completed.addLast(r); return true;
        }
        return false;
    }
    ...
    public void cancelAll(String reason) {
        for (TaskRecord r : pending) {
            ...
            r.markFailure(TaskFailureCode.CANCELLED, reason);
            r.setResult(TaskResult.cancelled(reason, TaskFailureCode.CANCELLED.code(), Map.of("reason", reason)));
            completed.addLast(r);
        }
        pending.clear();
