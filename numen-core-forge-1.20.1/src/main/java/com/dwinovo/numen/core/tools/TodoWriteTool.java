...
public final class TodoWriteTool implements NumenTool {
    ...
    public String description() {
        return """
                ...
                - `blocked` — cannot progress until a stated dependency or owner decision is resolved
    ...
    public Map<String, Object> parameterSchema() {
        return Schema.object()
                .objectArray("todos", "The complete updated todo list (replaces the previous one).", item -> item
                        ...
                        .enumStr("status", "One of: pending, in_progress, blocked, completed, cancelled.",
    ...
    public void invoke(ToolCall call) {
        try {
            ...
            String result = impl.todowrite(a.todos());
            if (com.google.gson.JsonParser.parseString(result).getAsJsonObject().get("success").getAsBoolean()) {
                java.util.ArrayList<AutonomyMemory.Goal> goals = new java.util.ArrayList<>();
                int index = 1;
                for (AgentTools.Todo todo : a.todos()) {
                    goals.add(new AutonomyMemory.Goal("goal-" + index++, todo.content(), todo.status(),
                            todo.priority(), "", System.currentTimeMillis()));
                }
                AutonomyTool.memory(call).replaceGoals(goals);
            }
