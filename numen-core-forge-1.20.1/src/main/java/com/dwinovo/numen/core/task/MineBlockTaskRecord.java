...
public final class MineBlockTaskRecord extends TaskRecord {
    ...
    private int mined = 0;
    private int inventoryBaseline = -1;
    ...
    public MineBlockTaskRecord(String toolCallId, long deadlineGameTime,
                               Set<Block> targets, int count, int maxRadius, String label,
                               int restoredMined, int inventoryBaseline) {
        this(toolCallId, deadlineGameTime, targets, count, maxRadius, label);
        this.mined = Math.max(0, Math.min(count, restoredMined));
        this.inventoryBaseline = inventoryBaseline;
    }
    ...
    public int getInventoryBaseline() { return inventoryBaseline; }
    public void setInventoryBaseline(int inventoryBaseline) { this.inventoryBaseline = Math.max(0, inventoryBaseline); }
